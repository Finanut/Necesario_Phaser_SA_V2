export default class Level3 extends Phaser.Scene {
    ...
    preload() {
        // Load assets for Level 3 theme - Space
    ...
    create() {
        ...
        // Create platforms - challenging layout for Level 3
        this.platforms.create(150, 450, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(300, 350, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(500, 350, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(700, 450, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(400, 250, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(200, 150, 'ground').setScale(0.5).refreshBody();
        this.platforms.create(600, 150, 'ground').setScale(0.5).refreshBody();
        ...
        // Create initial stars (5 at a time for highest difficulty)
        for (let i = 0; i < 5; i++) {
            this.createStar();
        }
        ...
        // Create initial bombs (level 3 is hardest)
        this.createBomb();
        this.createBomb();
        ...
        // Add level indicator
        this.levelText = this.add.text(680, 16, 'Level: 3/3', { 
            fontSize: '24px', 
            fill: '#fff',
            fontFamily: 'Arial',
            strokeThickness: 2,
    ...
    update() {
        ...
        // Handle player movement - reduced gravity for space theme
        ...
        // Handle jumping - higher jump in space theme
        if (this.cursors.up.isDown && this.player.body.touching.down) {
            this.player.setVelocityY(-380); // Higher jump for space level
            this.jumpSound.play();
        }
    ...
    createStar() {
        ...
        // Add physics properties - faster for level 3
        star.setBounceY(Phaser.Math.FloatBetween(0.8, 1));
        star.setVelocityX(Phaser.Math.Between(-100, 100)); // Faster stars
        star.setScale(0.5); // Slightly larger stars
        star.setCollideWorldBounds(true);
        star.setVelocity(Phaser.Math.Between(-150, 150), 40); // Fastest in level 3
    }
    ...
    createBomb() {
        ...
        bomb.setScale(0.5); // Slightly larger bombs
        bomb.setBounce(1);
        bomb.setCollideWorldBounds(true);
        bomb.setVelocity(Phaser.Math.Between(-250, 250), 40); // Fastest bombs in level 3
    }
    ...
    collectStar(player, star) {
        ...
        // Create a bomb every 3 stars (most frequent of all levels)
        if (this.starsCollected % 3 === 0) {
            this.createBomb();
        }
        ...
        if (this.starsCollected >= this.targetStars) {
            ...
            // Pause for a moment before transitioning to win scene
            this.time.delayedCall(2000, () => {
                this.scene.start('Congratulations');
            });
            
            // Display level complete message
            const completeText = this.add.text(400, 300, 'FINAL LEVEL COMPLETE!', {
                fontSize: '42px',
                fill: '#fff',
                fontFamily: 'Arial',
                strokeThickness: 6,
